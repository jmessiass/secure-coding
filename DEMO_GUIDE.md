...
Ao final, perguntar à audiência:
1. "Quantas dessas vulnerabilidades existem no código que vocês mantêm hoje?"
2. "O pipeline de CI/CD de vocês pegaria alguma dessas?"
3. "Vocês têm logs suficientes para detectar um ataque?"
4. "No ataque de 22/03, R$ 100–230M foram desviados via Pix. As APIs de integração no seu sistema têm velocity checks e validação de ownership?"
5. "Se um fornecedor terceiro no seu pipeline for comprometido (como a C&M Software), vocês detectariam?"

---

## Correlação com o Ataque ao BTG (22/03/2026) — OWASP Top 10:2025

> **Referência:** [Jovem Pan — BTG é alvo de ataque hacker e suspende operações via Pix](https://jovempan.com.br/noticias/economia/btg-e-alvo-de-ataque-hacker-e-suspende-operacoes-via-pix.html)

Use esta tabela para conectar cada demo ao incidente real durante o treinamento:

| Demo | Vuln (2025) | Conexão com o ataque BTG |
|------|------|--------------------------|
| Demo 1 | A05 — Injection | Se a integração C&M → Pix construía queries sem parametrização, atacantes podiam manipular transações |
| Demo 2 | A01 — Broken Access Control | Sem validação de ownership na API de transação, qualquer chamada autenticada poderia desviar para contas arbitrárias |
| Demo 3 | A02 — Security Misconfiguration | Endpoints internos expostos no intermediário (C&M) podem ter sido a porta de entrada. Subiu para #2 em 2025 |
| Demo 4 | A04 — Cryptographic Failures | Se credenciais de integração usavam hashing fraco ou eram armazenadas inseguramente, comprometimento ficou mais fácil |
| Demo 5 | A01 — Broken Access Control (SSRF) | Usar o sistema da C&M como "proxy" para acessar APIs internas do BTG = SSRF (absorvido em A01 na versão 2025) |
| Demo 6 | A07 — Auth Failures | Sem rate limit e MFA na API de transação, atacantes puderam escalar o volume de desvios |
| Demo 7 | A09 — Logging & Alerting | "Atividades atípicas" detectadas só de manhã. Renomeado para "Alerting" em 2025: logs sem alertas = detecção lenta |
| — | A03 — Supply Chain 🆕 | **O mais relevante ao caso.** C&M Software = fornecedor comprometido. Esta categoria subiu para #3 em 2025 — votada #1 pela comunidade |
| — | A10 — Exceptional Conditions 🆕 | **Nova categoria.** Race conditions em transações, failing open sem rollback = cenário #3 descrito pela própria OWASP |
